<template>
  ...
  export default {
    ...
    handleRemove() {
      ...
      // 回显数据
      async getdetailinfo(){
        // let res = await GetadvertInfo({id:this.$route.query.id})
        // if(res.status == 200){
        //     // let {Isdelete} = res.datalist
        //   this.ruleForm =  res.datalist
        // }else{
      ...
      submitForm(formName) {
        this.$refs[formName].validate(async(valid) => {
          if (valid) {
            this.loading = true;
            let res = await UpdateAd({...this.ruleForm})
            if(res.status == 200){
